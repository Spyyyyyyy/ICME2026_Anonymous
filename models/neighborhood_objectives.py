...
class PatchNCELoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        if self.nce_includes_all_negatives_from_minibatch:
            ...
            batch_dim_for_bmm = self.patch_batch_size
        ...
        diagonal = torch.eye(npatches, device=self.device, dtype=self.mask_dtype)[None, :, :]
        l_neg_curbatch.masked_fill_(diagonal, -10.0)
        l_neg = l_neg_curbatch.view(-1, npatches)

        out = torch.cat((l_pos, l_neg), dim=1) / self.nce_T

        loss = self.cross_entropy_loss(out, torch.zeros(out.size(0), dtype=torch.long, device=self.device))
...
class AdaptiveSupervisedPatchNCELoss(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.total_epochs = opt['train.params.n_epochs'] + opt['train.params.n_epochs_decay']
        self.nce_includes_all_negatives_from_minibatch = opt['train.params.loss.gan_loss.nce_includes_all_negatives_from_minibatch']
        self.patch_batch_size = opt['train.params.patch_batch_size_per_gpu']
        self.nce_T = opt['train.params.loss.gan_loss.nce_T']
    ...
    def forward(self, feat_q, feat_k, current_epoch=-1):
        ...
        if self.opt['train.params.loss.gan_loss.asp_loss_mode'] == 'none':
            return loss

        scheduler, lookup = self.opt['train.params.loss.gan_loss.asp_loss_mode'].split('_')[:2]
        # Compute scheduling
