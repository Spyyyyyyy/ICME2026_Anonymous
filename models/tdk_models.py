...
class TDKModel(nn.Module):
    def __init__(self, opt, device):
        super(TDKModel, self).__init__()
        ...
        self.lr_E = opt['train.params.optimizer.lr.lr_E']
        ...
        for i in range(1, 2 + 1): # 所使用netG下采样次数为2
            self.feat_layers.append(4 + i * 3 - 1)

        feats_nc = 0
        for nc in range(1, 2 + 1):
            feats_nc += opt['train.model.generator.ngf'] * (2 ** nc)
        self.netE = cut_networks.define_E(feats_nc, 1, opt)

        self.criterionGAN = GANLoss(opt['train.params.loss.gan_loss.gan_mode'], device=self.device)
        self.criterionL1 = torch.nn.L1Loss().to(self.device)
        self.criterionMask = torch.nn.L1Loss().to(self.device)
        self.criterionEstimator = torch.nn.MSELoss().to(self.device)
        self.criterionNuclei = torch.nn.MSELoss().to(self.device)
    ...
    #     model_input = {'src': data[0], 'dst': data[1]}
    ...
    def compute_G_loss(self):
        ...
        if self.lambda_asp > 0:
            self.loss_ASP = self.compute_NCE_loss(self.real_B, self.fake_B)
        else:
            self.loss_ASP = 0.0
