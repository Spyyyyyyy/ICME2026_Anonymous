...
class cond_CPTModel(nn.Module):
    def __init__(self, opt, device):
        super(cond_CPTModel, self).__init__()
    ...
    def data_dependent_initialize(self, data):
        ...
        model_input = {'src': data[0], 'dst': data[3], 'cond': data[6], 'current_epoch': 0}
        self.set_input(model_input)
        self.real_A = self.real_A[:bs_per_gpu]
        self.real_B = self.real_B[:bs_per_gpu]
        self.cond = self.cond[:bs_per_gpu]
    ...
    def set_input(self, input):
        ...
        self.cond = input['cond'].to(self.device)
    ...
    def compute_G_loss(self):
        ...
        # NCE loss: NCE(A, G(A))
        if self.lambda_NCE > 0.0:
            self.loss_NCE_A = self.compute_NCE_loss(self.real_A_cond, self.fake_B_cond)
        else:
            self.loss_NCE_A = 0.0

        # NCE-IDT loss: NCE(B, G(B))
        if self.nce_idt and self.lambda_NCE > 0.0:
            self.loss_NCE_B = self.compute_NCE_loss(self.real_B_cond, self.idt_B_cond)
        else:
            self.loss_NCE_B = 0

        # NCE between the noisy pairs (fake_B and real_B)
        if self.lambda_asp > 0:
            self.loss_ASP = self.compute_NCE_loss(self.real_B_cond, self.fake_B_cond, paired=True)
        else:
            self.loss_ASP = 0.0
    ...
    def forward(self):
        ...
        # 拼接图像与掩膜作为网络输入
        self.real_A_cond = torch.cat((self.real_A, self.cond), dim=1)
        self.real_B_cond = torch.cat((self.real_B, self.cond), dim=1)
        self.real = torch.cat((self.real_A_cond, self.real_B_cond), dim=0) if self.nce_idt else self.real_A_cond
        ...
        self.fake_B_cond = torch.cat((self.fake_B, self.cond), dim=1)
        self.idt_B_cond = torch.cat((self.idt_B, self.cond), dim=1)
