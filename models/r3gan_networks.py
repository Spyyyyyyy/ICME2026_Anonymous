...
from models.Resamplers import InterpolativeUpsampler, InterpolativeDownsampler
from models.FusedOperators import BiasedActivation
...
# 输出为图像，不需要从点映射为初始的空间特征图
class GenerativeBasis(nn.Module):
    def __init__(self, InputDimension, OutputChannels):
        super(GenerativeBasis, self).__init__()
        
        self.conv = MSRInitializer(nn.Conv2d(InputDimension, OutputChannels, kernel_size=3, stride=1, padding=1, bias=False))
...
class DiscriminativeBasis(nn.Module):
    def __init__(self, InputChannels, OutputDimension):
        ...
        self.conv = MSRInitializer(nn.Conv2d(InputChannels, OutputDimension, kernel_size=3, stride=1, padding=1, bias=False))
